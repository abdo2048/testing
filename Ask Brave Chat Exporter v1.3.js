...
// @version      1.3.0
// @description  Export Ask Brave conversations to Markdown and/or HTML (API mode: no clipboard needed — works on long conversations & Firefox)
// @author       abdo2048
...
(function() {
    ...
    // =============================================
    const CONFIG = {
        COPY_DELAY: 200, // ms between copy operations (clipboard fallback mode)
        MAX_TITLE_LENGTH: 40,
        EXPORT_MODE: 'auto',           // 'auto' | 'api' | 'clipboard'
        INCLUDE_RESOURCES: 'cited'     // 'none' | 'cited' | 'all'  (Step 3 will use this)
    };

    // =============================================
    // API-MODE STATE CAPTURE (Step 1)
    // Passive network interceptor installed at document-start.
    // It never modifies requests; it only clones responses it recognizes.
    // Sources of truth (verified against live captures):
    //   - SSE stream events from /api/tap/v1/stream(_multimodal)?...
    //   - JSON state from /api/tap/v1/get_current_state?...
    // =============================================
    ...
    function abxHandleEvent(evt) {
        try {
            ...
            switch (evt.type) {
                case 'question':
                    if (typeof evt.text === 'string') turn.question = evt.text;
                    break;
                case 'answer':
                    if (typeof evt.content === 'string') turn.answerMd += evt.content;
                    break;
                case 'augment_with_inline_citation': {
                    const c = evt.citation || {};
                    const url = c.url || c.link || '';
                    if (!url) break;
                    const key = String(c.id || url);
                    if (!turn.citationsById[key]) {
                        turn.citationsById[key] = {
                            number: c.number,
                            url: url,
                ...
                case 'tool_use':
                    if (typeof evt.name === 'string' && evt.name.indexOf('augment_with_') === 0) {
                        turn.augmentations.push({
                            type: evt.name,
                            query: (evt.arguments && evt.arguments.q) || ''
                        });
                    }
                    break;
                case 'rag':
        ...
        try {
            ...
            const walk = (o, depth) => {
                ...
                if (typeof o.type === 'string' &&
                    (o.type === 'question' || o.type === 'answer' ||
                     o.type === 'augment_with_inline_citation' || o.type === 'tool_use' ||
                     o.type === 'rag' || o.type === 'final')) {
                    found.push(o);
                    return;
                }
    ...
    function PatchedXHR() {
        ...
        xhr.addEventListener('load', function() {
            try {
                ...
                } else if (_url.indexOf('/api/tap/v1/get_current_state') !== -1) {
    ...
    function buildConversationFromApiState() {
        ...
        if (!conv || !conv.turns || conv.turns.length === 0) return null;

        const conversation = [];
        let qnum = 0;
        for (let i = 0; i < conv.turns.length; i++) {
            const t = conv.turns[i];
            const hasQ = typeof t.question === 'string' && t.question.trim() !== '';
            const hasA = typeof t.answerMd === 'string' && t.answerMd.trim() !== '';
            if (!hasQ && !hasA) continue;
            qnum++;
            if (hasQ) conversation.push({ type: 'user', content: t.question.trim(), index: qnum });
            if (hasA) conversation.push({ type: 'assistant', content: t.answerMd.trim(), index: qnum });
        }
        if (conversation.length === 0) return null;

        // Attach per-turn sources metadata for later steps (currently unused by generators)
        conversation.sourcesByTurn = conv.turns.map(t => ({
            index: t.index,
    ...
    function showExportDialog() {
        ...
        // API-mode title fallback: first captured question (works even if DOM virtualized)
        ...
        if (CONFIG.EXPORT_MODE === 'clipboard') {
            ...
            sourceStatus = '⚠️ No API data captured yet for this conversation — will fall back to clipboard mode.';
        }
...
`;
    ...
    async function startExport(title, exportMd, exportHtml) {
        ...
        try {
            ...
            // ---- Mode selection (Step 1: API-first, clipboard fallback) ----
            ...
            if (mode === 'api' || mode === 'auto') {
                ...
                if (!conversation && mode === 'auto') {
                    console.warn('[ABX] No API state captured for this conversation; falling back to clipboard mode.');
                }
            }
    ...
    function generateMarkdown(title, conversation, usedMode) {
        ...
        if (usedMode === 'api') {
            ...
            md += '**Method:** Clipboard copy buttons\n';
        }
        md += '\n---\n';
